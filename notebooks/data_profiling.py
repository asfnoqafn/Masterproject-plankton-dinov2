...
def open_and_measure_data(data_path, archive_type):
    heights = []
    widths = []
    num_images = 0
    ...
    if archive_type == 'lmdb':
        ...
        #  cursor_labels = txn_labels.cursor()        
            ...
            for (img_key, img_value) in cursor_imgs:
                ...
                num_images += 1
                if True: #width < 400 and height < 400:
                    widths.append(width)
                    heights.append(height)
        ...
        with zipfile.ZipFile(data_path, 'r') as zip_file:
            ...
            for img_file in image_files:
                with zip_file.open(img_file) as img_data:
                    ...
                    width, height = img.size
                    num_images += 1
                    if True: #width < 400 and height < 400:                        widths.append(width)
                        heights.append(height)
                        widths.append(width)
    ...
    create_heatmap(widths, heights, num_images, bins=1000)
