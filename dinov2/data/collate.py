...
def collate_data_and_cast(
    ...
    if use_ch_patch_embed and exists(c):
        ...
        mask_indices_list = [
            coll_mask_b.flatten().nonzero().flatten() for coll_mask_b in masks_list
        ]
        masks_weight = [
            (1 / coll_mask_b.sum(-1).clamp(min=1.0))
            .unsqueeze(-1)
            .expand_as(coll_mask_b)[coll_mask_b]
            for coll_mask_b in masks_list
        ]
        upperbound = [
            m_idx.shape[0] if m_idx.shape[0] > upperbound else upperbound
