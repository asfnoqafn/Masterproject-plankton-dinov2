...
def process_image(img_path, args, model, device, autocast_dtype):
    ...
    for j in range(nh):
        attn_map = cls_to_patch_attn[j]  # shape (H, W), float
        attn_map = (attn_map - attn_map.min()) / (attn_map.max() - attn_map.min() + 1e-8)  # normalize to [0,1]

        attn_map_uint8 = (attn_map * 255).astype(np.uint8)
        attn_rgb = cv2.applyColorMap(attn_map_uint8, cv2.COLORMAP_JET)  # shape (H, W, 3), uint8, BGR

        # Convert BGR to RGB
        attn_rgb = cv2.cvtColor(attn_rgb, cv2.COLOR_BGR2RGB)

        fname = os.path.join(args.output_dir2, f"{img_name}_cls_attn_to_patch_head{j}_rgb.png")
        Image.fromarray(attn_rgb).save(fname)
        print(f"{fname} saved.")
